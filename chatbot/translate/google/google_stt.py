...
def transcribe_chunk(client, project_id, location, config, chunk_number, chunk, chunk_duration):
    with langfuse.start_as_current_observation(
        ...
        input={"chunk_number": chunk_number, "location": location},
    ) as gen:
        ...
        try:
            ...
            logger.error(
                ...
                chunk_number, e, location, request.recognizer, exc_info=True,
            )
            traceback.print_exc()
...
def transcribe_multiple_languages_v2(
    ...
    with langfuse.start_as_current_observation(
        ...
        input={"language_codes": language_codes, "voice_provider_id": getattr(voice_provider, 'id', None)},
    ) as s:
        try:
