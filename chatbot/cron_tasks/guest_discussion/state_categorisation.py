...
def _translate_location_to_english(location_text: str) -> Optional[str]:
    ...
    try:
        ...
        if not detect_response or detect_response.get("status") != 200:
            logger.warning(
                f"Language detection failed for location '{location_text}': {detect_response}"
            )
            return None

        detected_lang = detect_response.get("content")
        logger.info(
            f"Detected language '{detected_lang}' for location '{location_text}'"
        )

        if detected_lang not in SUPPORTED_LANGUAGES:
            logger.warning(
                f"Detected language '{detected_lang}' not in supported set {SUPPORTED_LANGUAGES}; skipping translation"
            )
            return None
        ...
        if not chaupal_bot:
            logger.warning(
                "CompanyBot '/shikshalokam_chaupal' not found; skipping translation"
            )
            return None
        ...
        if not voice:
            logger.warning(
                f"No Google TextToText Voice for language='{detected_lang}' on '/shikshalokam_chaupal'; skipping translation"
            )
            return None
        ...
        logger.info(
            f"Translating location '{location_text}' from '{detected_lang}' to English"
        )
...
def _matches(pattern, text: str) -> bool:
    ...
    if isinstance(pattern, list):
        return all(_matches(p, text) for p in pattern)
    escaped = re.escape(pattern)
    return bool(
        re.search(r"(?<![a-zA-Z])" + escaped + r"(?![a-zA-Z])", text, re.IGNORECASE)
    )
...
def _match_location(
    location_text: str, mapping_data: Dict[str, Any]
...
def _match_organisation(
    org_text: str, mapping_data: Dict[str, Any]
) -> Dict[str, Optional[str]]:
    """
    ...
    org_to_districts: Dict[str, List[str]] = mapping_data.get(
        "organisation_to_district_mapping", {}
    )
    district_to_org: Dict[str, Dict[str, Optional[str]]] = mapping_data.get(
        "district_to_organisation_mapping", {}
    )
...
def categorise_from_text(
    text: str, mapping_data: Dict[str, Any]
) -> Dict[str, Optional[str]]:
    """
...
def categorise_story(
    story: Story, mapping_data: Dict[str, Any]
        ...
        if story.other_params:
            ...
            if isinstance(english_json, dict):
                ...
                if val and isinstance(val, str) and _is_likely_english(val):
                    ...
                    logger.info(
                        f"Story {story.id}: using existing english_json.location '{english_json_loc}'"
                    )
        if english_json_loc:
            ...
            if translated:
                ...
                logger.info(
                    f"Story {story.id}: persisted translated location '{translated}' to english_json"
                )
                location_text = translated
            else:
                logger.warning(
                    f"Story {story.id}: translation unavailable; proceeding with original text '{location_text}'"
                )
        ...
        with transaction.atomic():
            for update in updates:
                ...
                try:
                    ...
                    logger.info(
                        f"Updated story {story_id}: state={update['state']}, district={update['district']}"
                    )
...
def run_state_categorisation(
    story_queryset=None, mapping_data: Dict[str, Any] = None, company_bot=None
) -> Dict[str, Any]:
    """
    ...
    if mapping_data is None:
        if company_bot is None:
            logger.error(
                "company_bot required for mapping data; aborting state categorisation"
            )
    ...
    if story_queryset is None:
        ...
        ).filter(Q(state__isnull=True) | Q(district__isnull=True))
    ...
    for story in stories:
        try:
            ...
            if categorisation["state"] or categorisation["district"]:
                ...
                story.save(update_fields=["state", "district"])
                logger.info(
                    f"Updated story {story.id}: state={categorisation['state']}, district={categorisation['district']}"
                )
                processed += 1
            else:
                skipped += 1
...
def run_cron() -> None:
    ...
    try:
        ...
        logger.error(
            "CompanyBot with route '/state-classification-guest-discussion' not found"
        )
        return
    except CompanyBot.MultipleObjectsReturned:
        logger.error(
            "Multiple CompanyBots with route '/state-classification-guest-discussion' found"
        )
        return
...
def backfill_state_from_district(
    mapping_data: Dict[str, Any] = None, company_bot=None
) -> Dict[str, Any]:
    """
    ...
    if mapping_data is None:
        if company_bot is None:
            logger.error(
                "company_bot required for mapping data; aborting state backfill"
            )
            return {"processed": 0, "skipped": 0, "failed": []}
        mapping_data = load_mapping_from_bot(company_bot)
    ...
    for state_name, districts_map in mapping_data.get(
        "district_to_organisation_mapping", {}
    ).items():
    ...
    logger.info(
        f"Starting state backfill for {len(stories)} stories with district but no state"
    )
    ...
    for story in stories:
        try:
            ...
            if inferred_state:
                ...
                logger.warning(
                    f"Story {story.id}: district '{story.district}' not found in mapping"
                )
                skipped += 1
        except Exception as e:
...
def run_for_story_ids(
    story_ids: List[int], mapping_data: Dict[str, Any] = None, company_bot=None
) -> Dict[str, Any]:
    stories = Story.objects.filter(id__in=story_ids)
    return run_state_categorisation(
        story_queryset=stories, mapping_data=mapping_data, company_bot=company_bot
    )


def run_for_date_range(
    start_date, end_date, mapping_data: Dict[str, Any] = None, company_bot=None
) -> Dict[str, Any]:
