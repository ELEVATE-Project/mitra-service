...
def format_message_as_per_bedrock_format(chats, intro=None, other_info=None):
    ...
    if intro:
        if other_info:
            ...
            initial_msg = "Hello"
        messages = [
            {"role": "user", "content": [{"text": initial_msg}]},
            {"role": "assistant", "content": [{"text": intro}]},
        ]
    else:
        messages = []
    for chat in chats:
        ...
        if chat_receiver == ai_user.get("id"):
            ...
            messages.append({"role": "assistant", "content": [{"text": chat_message}]})
...
def get_guided_chat(company_bot, company_chats, intro=None, other_info=None):
    ...
    if company_bot.provider == LLMProvider.BEDROCK_CONVERSE:
        messages = format_message_as_per_bedrock_format(
            chats=company_chats, intro=intro, other_info=other_info
        )
...
def convert_llama_to_openai_tool(llama_tool_call):
    try:
        tool_spec = (
            llama_tool_call.get("toolConfig", {})
            .get("tools", [])[0]
            .get("toolSpec", {})
        )
        ...
        if not tool_spec:
            raise ValueError(
                "Invalid toolSpec structure in the provided Llama tool call."
            )
