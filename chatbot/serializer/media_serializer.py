...
class MediaDetailSerializer(serializers.ModelSerializer, S3UrlMixin):
    ...
    def _get_references_and_associated_documents(self, obj):
        ...
        for child in children:
            ...
            # If it's a source document and has VISIBLE children, show those grandchildren instead
            if is_source_doc and child.subdocuments.filter(display_mode=FileDisplayMode.VISIBLE).exists():
                ...
                for grandchild in grandchildren:
                    ...
                    # Create HTML link for grandchild
                    references_html.append(
                        f'<div><a class="text-blue-600 underline underline-offset-2" '
                        f'href="/mohini/shikshagraha-commons/{grandchild.id}" target="_blank" '
                        f'rel="noopener noreferrer">{grandchild_title}</a></div>'
                    )
            else:
                # Show the child document itself (not a source doc, or source doc without children)
                # Get title for child
                ...
                references_html.append(
                    f'<div><a class="text-blue-600 underline underline-offset-2" '
                    f'href="/mohini/shikshagraha-commons/{child.id}" target="_blank" '
