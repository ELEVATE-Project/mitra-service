...
class AsyncSocketConsumer(AsyncBaseConsumer):
    ...
    async def receive(self, text_data):
        try:
            ...
            if (
                ...
                and text_data_json
                and text_data_json.get("text")
            ):
                chat_session = await database_sync_to_async(
                    lambda: (
                        ChatSession.objects.filter(session=self.session_id)
                        .order_by("-created_at")
                        .first()
                    )
                )()
                ...
                if (
                    chat_session
                    and self.company_bot
                    and self.company_bot.bot_type == CompanyBotTypeChoices.STATE_MACHINE
                ):
                    try:
    ...
    def create_chat_session(
        self, session_id, profile, company_bot, ip_address, user_id
    ):
        step_number = 1
    ...
    def translate_message(self, message):
        try:
            ...
            if (
                state_machine
                and state_machine.text_conversion_type
                == TextConversionType.TRANSLITERATE
            ):
                ...
                response = transliterate_text(
                    voice_provider=transliterate_voice_provider,
